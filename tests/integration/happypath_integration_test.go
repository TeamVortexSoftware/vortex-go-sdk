...
// TestIntegration tests the full invitation flow: Create -> Get -> Accept
func TestIntegration(t *testing.T) {
	...
	fmt.Println("\n--- Starting Go SDK Integration Test ---")
	...
	fmt.Println("Step 2a: Getting invitation by ID...")
	invitation, err := publicClient.GetInvitation(invitationID)
	if err != nil {
		t.Fatalf("Failed to get invitation by ID: %v", err)
	}
	if invitation.ID != invitationID {
		t.Fatalf("Retrieved invitation ID %s does not match expected ID %s", invitation.ID, invitationID)
	}
	fmt.Println("✓ Retrieved invitation by ID successfully")

	// Step 2b: Get invitations by target
	fmt.Println("Step 2b: Getting invitations by target...")
	...
	if !foundInList {
		t.Fatal("Invitation not found in list returned by target")
	}
	fmt.Println("✓ Retrieved invitations by target successfully and verified invitation is in list")

	// Step 3: Accept invitation
	fmt.Println("Step 3: Accepting invitation...")
	target := vortex.InvitationTarget{
		Type:  "email",
		Value: userEmail,
	}
	result, err := publicClient.AcceptInvitations([]string{invitationID}, target)
	if err != nil {
		t.Fatalf("Failed to accept invitation: %v", err)
	}
	if result == nil {
		t.Fatal("Accept invitation returned nil result")
	}
	fmt.Println("✓ Accepted invitation successfully")

	fmt.Println("--- Go SDK Integration Test Complete ---")
}
